...
const rows = {
  ...
  toprated: document.getElementById("row-toprated"),
  now: document.getElementById("row-now"),
...
const buildCardHTML = (item) => {
  ...
  const href = item.media_type === "tv" || item.first_air_date
    ? `https://www.themoviedb.org/tv/${item.id}`
    : `https://www.themoviedb.org/movie/${item.id}`;

  return `
    <a class="card" href="${href}" target="_blank" rel="noopener">
      ${poster ? `<img src="${poster}" alt="${title}">` : `<div style="height:220px;display:flex;align-items:center;justify-content:center;color:#888;background:#101018">No Poster</div>`}
      <div class="meta">
...
const loadHero = async () => {
  ...
  if (movie.backdrop_path) {
    heroEls.backdrop.style.backgroundImage = `url(${IMG(movie.backdrop_path, 1280)})`;
  }
};
...
const loadGenres = async () => {
  ...
  genreMenu.querySelectorAll("a").forEach(a => {
    a.addEventListener("click", async (e) => {
      ...
      fillRow(rows.genre, page1.results);
